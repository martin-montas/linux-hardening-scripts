...
SPACE_LEFT_ACTION=$(grep -P '^space_left_action\s' /etc/audit/auditd.conf)
ACTION_MAIL_ACCT=$(grep -P '^action_mail_acct\s' /etc/audit/auditd.conf)
ADMIN_SPACE_LEFT_ACTION=$(grep -P '^admin_space_left_action\s' /etc/audit/auditd.conf)
...
if [ "$COUNT" -eq 3 ]; then
    ...
    echo -e "${RED}[!]${RESET} some of the required parameters were not set for Auditd"
    exit 1
fi
